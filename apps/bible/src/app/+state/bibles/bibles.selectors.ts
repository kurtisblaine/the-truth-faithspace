...
export const selectBiblesState =
  createFeatureSelector<BiblesState>(BIBLES_FEATURE_KEY);

const { selectAll, selectEntities } = biblesAdapter.getSelectors();

export const selectBiblesLoaded = createSelector(
  selectBiblesState,
  (state: BiblesState) => state.loaded
);

export const selectBiblesError = createSelector(
  selectBiblesState,
  (state: BiblesState) => state.error
);

export const selectAllBibles = createSelector(
  selectBiblesState,
...
export const selectAllGroupedLanguages = createSelector(
  ...
  (bibles, languages) =>
    bibles
      .reduce(
        (sortedBibles, book) => {
          const foundLanguage = sortedBibles.find(
...
export const selectAllGroupedScripts = createSelector(
  ...
  (bibles, scripts) =>
    bibles
      .reduce(
        (sortedBibles, book) => {
          const foundScript = sortedBibles.find(
